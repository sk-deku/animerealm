...
try:
    ...
    if ADMIN_IDS_STR:
        try: ADMIN_IDS = [int(admin_id.strip()) for admin_id in ADMIN_IDS_STR.split(',') if admin_id.strip()]; if not ADMIN_IDS: main_logger.warning("ADMIN_IDS parsed empty."); else: main_logger.info(f"Parsed ADMIN_IDS: {ADMIN_IDS}");
