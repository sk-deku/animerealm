...
from configs import settings # Import your settings
...
class Database:
    ...
    # --- User Management ---
    async def add_or_update_user(self, user_id: int, first_name: str, username: str | None = None, referred_by: int | None = None):
        """Adds a new user or updates existing user's first_name/username if changed."""
        ...
        if not user_data: # New user
            ...
            new_user_doc = {
                ...
                "last_token_earn_reset_date": now.All right, let's proceed with the `database/mongo_db.py` file. This file will set up the MongoDB connection and provide helper functions or classes for interacting with your collections.

For simplicity and to avoid overly complex class structures initially, I'll provide functions for each major collection. You could later refactor these intodate(), # Store as date object
                "notification_preferences": {"watchlist_new_episode": True, "quality_update": False}, # Default preferences
                "is_banned": False
            }
            try:
                await self.users_collection.insert_one(new_user_doc)
                logger.info(f"New user {user_id} added with {initial_tokens} tokens.")
                return {"new": True, "tokens_awarded": initial_tokens, "user_doc": new_user_doc}
            except Exception as e:
                logger.error(f"Error adding new user {user_id}: {e}")
                return None
        else: # Existing user
            update_fields = {"last_active_date": now}
            if user_data.get("first_name") != first_name:
                update_fields["first_name"] = first_name
            if user_data.get("username") != username:
                update_fields["username"] = username

            # Check and reset daily token earn limit
            if user_data.get("last_token_earn_reset_date") != now.date():
                update_fields["tokens_earned_today"] = 0
                update_fields["last_token_earn_reset_date"] = now.date()

            try:
                await self.users_collection.update_one({"telegram_id": user_id}, {"$set": update_fields})
                # Refresh user_data after update
                user_data.update(update_ classes if you prefer a more OOP approach for database interactions.
